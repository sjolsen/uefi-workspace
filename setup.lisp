...
;; TODO: put this in the refinery repository and symbol-call it from here
(defun regenerate-test-data ()
  ...
  (let* ((test-base "~/Code/uefi-workspace/refinery/BoraxPkg/Test/BoraxRuntimeTest/")
         (test-files `(("TestFileIA32.bxo" . ,(uiop:symbol-call :borax-runtime/memory :make-memory-model 32 :little-endian))
                       ("TestFileX64.bxo"  . ,(uiop:symbol-call :borax-runtime/memory :make-memory-model 64 :little-endian)))))
    (loop for (basename . memory-model) in test-files
          for path = (concatenate 'string test-base basename)
          do (uiop:symbol-call :borax-runtime/object-file-test :make-test-file path memory-model))))
